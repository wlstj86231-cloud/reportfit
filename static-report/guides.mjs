...
const cards = guides.map((guide) => `<article><p>정산 원리</p><h2><a href="/guides/${guide.slug}/">${esc(guide.title)}</a></h2><span>${esc(guide.description)}</span><a href="/guides/${guide.slug}/">계산 순서 보기 <i data-lucide="arrow-right"></i></a></article>`).join("") + `<article><p>직접 계산</p><h2><a href="/tools/used-tractor-total-cost/">중고 트랙터 두 후보의 구입 총비용</a></h2><span>매입가 외 운송·정비·작업기 견적을 같은 범위에 넣어 비교합니다.</span><a href="/tools/used-tractor-total-cost/">계산기 열기 <i data-lucide="arrow-right"></i></a></article>`;
